...
uint8_t RdByte(VL53LMZ_Platform *p_platform, uint16_t RegisterAdress, uint8_t *p_value) {
    ...
    status = HAL_I2C_Master_Transmit(VL53_I2C_HANDLE, p_platform->address, data_write, 2, 100);
    status = HAL_I2C_Master_Receive(VL53_I2C_HANDLE, p_platform->address, data_read, 1, 100);
    *p_value = data_read[0];
...
uint8_t WrByte(VL53LMZ_Platform *p_platform, uint16_t RegisterAdress, uint8_t value) {
    ...
    status = HAL_I2C_Master_Transmit(VL53_I2C_HANDLE, p_platform->address, data_write, 3, 100);
...
uint8_t WrMulti(VL53LMZ_Platform *p_platform, uint16_t RegisterAdress, uint8_t *p_values, uint32_t size) {
    uint8_t status = HAL_I2C_Mem_Write(VL53_I2C_HANDLE, p_platform->address, RegisterAdress,
...
uint8_t RdMulti(VL53LMZ_Platform *p_platform, uint16_t RegisterAdress, uint8_t *p_values, uint32_t size) {
    ...
    status = HAL_I2C_Master_Transmit(VL53_I2C_HANDLE, p_platform->address, data_write, 2, 10);
    status += HAL_I2C_Master_Receive(VL53_I2C_HANDLE, p_platform->address, p_values, size, 400);
