...
static uint32_t g_output_bh_enable[NUM_OUTPUT_ENABLE_WORDS] =
  {
  0x00000007U, 0x00000000U, 0x00000000U, 0xC0000000U
  };
/**
...
static uint8_t _vl53lmz_poll_for_answer(VL53LMZ_Configuration *p_dev, uint8_t size, uint8_t pos, uint16_t address,
 ...
 */
static uint8_t _vl53lmz_poll_for_mcu_boot(VL53LMZ_Configuration *p_dev)
  {
...
static uint8_t _vl53lmz_send_offset_data(VL53LMZ_Configuration *p_dev, uint8_t resolution)
  {
  uint8_t status = VL53LMZ_STATUS_OK;
  ...
  uint8_t dss_4x4[] =
    {
    0x0F, 0x04, 0x04, 0x00, 0x08, 0x10, 0x10, 0x07
    };
  uint8_t footer[] =
    {
    0x00, 0x00, 0x00, 0x0F, 0x03, 0x01, 0x01, 0xE4
    };
  int8_t i, j;
  uint16_t k;

  (void) memcpy(p_dev->temp_buffer, p_dev->offset_data, VL53LMZ_OFFSET_BUFFER_SIZE);
    ...
    for (j = 0; j < (int8_t) 4; j++)
      ...
      for (i = 0; i < (int8_t) 4; i++)
        ...
        signal_grid[i + (4 * j)] = (signal_grid[(2 * i) + (16 * j) + (int8_t) 0]
            + signal_grid[(2 * i) + (16 * j) + (int8_t) 1] + signal_grid[(2 * i) + (16 * j) + (int8_t) 8]
            + signal_grid[(2 * i) + (16 * j) + (int8_t) 9]) / (uint32_t) 4;
        range_grid[i + (4 * j)] = (range_grid[(2 * i) + (16 * j)] + range_grid[(2 * i) + (16 * j) + 1]
            + range_grid[(2 * i) + (16 * j) + 8] + range_grid[(2 * i) + (16 * j) + 9]) / (int16_t) 4;
        }
      }
...
static uint8_t _vl53lmz_send_xtalk_data(VL53LMZ_Configuration *p_dev, uint8_t resolution)
  {
  uint8_t status = VL53LMZ_STATUS_OK;
  uint8_t res4x4[] =
    {
    0x0F, 0x04, 0x04, 0x17, 0x08, 0x10, 0x10, 0x07
    };
  uint8_t dss_4x4[] =
    {
    0x00, 0x78, 0x00, 0x08, 0x00, 0x00, 0x00, 0x08
    };
  uint8_t profile_4x4[] =
    {
    0xA0, 0xFC, 0x01, 0x00
    ...
    for (j = 0; j < (int8_t) 4; j++)
      ...
      for (i = 0; i < (int8_t) 4; i++)
        ...
        signal_grid[i + (4 * j)] = (signal_grid[(2 * i) + (16 * j) + 0] + signal_grid[(2 * i) + (16 * j) + 1]
            + signal_grid[(2 * i) + (16 * j) + 8] + signal_grid[(2 * i) + (16 * j) + 9]) / (uint32_t) 4;
        }
      }
  ...
  status |= WrMulti(&(p_dev->platform), 0x2cf8, p_dev->temp_buffer, VL53LMZ_XTALK_BUFFER_SIZE);
  status |= _vl53lmz_poll_for_answer(p_dev, 4, 1,
  VL53LMZ_UI_CMD_STATUS, 0xff, 0x03);
...
uint8_t vl53lmz_init(VL53LMZ_Configuration *p_dev)
  ...
  uint8_t pipe_ctrl[] =
    {
    VL53LMZ_NB_TARGET_PER_ZONE, 0x00, 0x01, 0x00
    };
  ...
  status |= WrMulti(&(p_dev->platform), 0, (uint8_t*) &VL53LMZ_FIRMWARE[0], 0x8000);
  status |= WrByte(&(p_dev->platform), 0x7fff, 0x0a);
  status |= WrMulti(&(p_dev->platform), 0, (uint8_t*) &VL53LMZ_FIRMWARE[0x8000], 0x8000);
  status |= WrByte(&(p_dev->platform), 0x7fff, 0x0b);
  status |= WrMulti(&(p_dev->platform), 0, (uint8_t*) &VL53LMZ_FIRMWARE[0x10000], 0x5000);
  status |= WrByte(&(p_dev->platform), 0x7fff, 0x01);
  ...
  status |= WrMulti(&(p_dev->platform), 0x2fd8, (uint8_t*) VL53LMZ_GET_NVM_CMD, sizeof(VL53LMZ_GET_NVM_CMD));
  status |= _vl53lmz_poll_for_answer(p_dev, 4, 0,
  VL53LMZ_UI_CMD_STATUS, 0xff, 2);
  status |= RdMulti(&(p_dev->platform), VL53LMZ_UI_CMD_START, p_dev->temp_buffer, VL53LMZ_NVM_DATA_SIZE);
  ...
  if (p_dev->revision_id == (uint8_t) REVISION_L8)
    ...
    status |= WrMulti(&(p_dev->platform), 0x2c34, p_dev->default_configuration, sizeof(VL53L8_DEFAULT_CONFIGURATION));
    }
  else
    {
    p_dev->default_configuration = (uint8_t*) VL53L7_DEFAULT_CONFIGURATION;
    status |= WrMulti(&(p_dev->platform), 0x2c34, p_dev->default_configuration, sizeof(VL53L7_DEFAULT_CONFIGURATION));
    }
  ...
  VL53LMZ_DCI_SINGLE_RANGE, (uint16_t) sizeof(single_range));

  uint8_t fw_flags[8];
  status |= vl53lmz_dci_read_data(p_dev, fw_flags, 0xE0C4, sizeof(fw_flags));
  p_dev->module_type = fw_flags[1];
//	printf("Module type = %u (0 for MZ, 1 for MZEVO, 2 for MZPlus)\n", fw_flags[1]);
...
uint8_t vl53lmz_set_power_mode(VL53LMZ_Configuration *p_dev, uint8_t power_mode)
  ...
  if (power_mode != current_power_mode)
    ...
    case VL53LMZ_POWER_MODE_WAKEUP:
      ...
      status |= _vl53lmz_poll_for_answer(p_dev, 1, 0, 0x06, 0x01, 1);
      if (stored_mode == 0x43) /* Only for deep sleep mode */
        {
    ...
    case VL53LMZ_POWER_MODE_SLEEP:
      ...
      status |= _vl53lmz_poll_for_answer(p_dev, 1, 0, 0x06, 0x01, 0);
      break;
...
uint8_t vl53lmz_set_ranging_frequency_hz(VL53LMZ_Configuration *p_dev, uint8_t frequency_hz)
  ...
  status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
  VL53LMZ_DCI_FREQ_HZ, 4, (uint8_t*) &frequency_hz, 1, 0x01);
...
uint8_t vl53lmz_set_integration_time_ms(VL53LMZ_Configuration *p_dev, uint32_t integration_time_ms)
  ...
  if ((integration < (uint32_t) 2) || (integration > (uint32_t) 1000))
    ...
    status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
    VL53LMZ_DCI_INT_TIME, 20, (uint8_t*) &integration, 4, 0x00);
    }
...
uint8_t vl53lmz_set_sharpener_percent(VL53LMZ_Configuration *p_dev, uint8_t sharpener_percent)
  ...
  if (sharpener_percent >= (uint8_t) 100)
    ...
    status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
    VL53LMZ_DCI_SHARPENER, 16, (uint8_t*) &sharpener, 1, 0xD);
    }
...
uint8_t vl53lmz_set_target_order(VL53LMZ_Configuration *p_dev, uint8_t target_order)
  ...
  if ((target_order == (uint8_t) VL53LMZ_TARGET_ORDER_CLOSEST)
    ...
    {
    status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
    VL53LMZ_DCI_TARGET_ORDER, 4, (uint8_t*) &target_order, 1, 0x0);
    }
  else
    {
    status |= VL53LMZ_STATUS_INVALID_PARAM;
...
uint8_t vl53lmz_enable_internal_cp(VL53LMZ_Configuration *p_dev)
  ...
  status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
  VL53LMZ_DCI_INTERNAL_CP, 16, (uint8_t*) &vcsel_bootup_fsm, 1, 0x0A);

  status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
  VL53LMZ_DCI_INTERNAL_CP, 16, (uint8_t*) &analog_dynamic_pad_0, 1, 0x0E);
...
uint8_t vl53lmz_dci_read_data(VL53LMZ_Configuration *p_dev, uint8_t *data, uint32_t index, uint16_t data_size)
  ...
  if ((data_size + (uint16_t) 12) > (uint16_t) VL53LMZ_TEMPORARY_BUFFER_SIZE)
    ...
    status |= WrMulti(&(p_dev->platform), (VL53LMZ_UI_CMD_END - (uint16_t) 11), cmd, sizeof(cmd));
    status |= _vl53lmz_poll_for_answer(p_dev, 4, 1,
    VL53LMZ_UI_CMD_STATUS, 0xff, 0x03);

    /* Read new data sent (4 bytes header + data_size + 8 bytes footer) */
    status |= RdMulti(&(p_dev->platform), VL53LMZ_UI_CMD_START, p_dev->temp_buffer, rd_size);
...
uint8_t vl53lmz_dci_write_data(VL53LMZ_Configuration *p_dev, uint8_t *data, uint32_t index, uint16_t data_size)
  ...
  uint8_t footer[] =
    {
    0x00, 0x00, 0x00, 0x0f, 0x05, 0x01, (uint8_t) ((data_size + (uint16_t) 8) >> 8), (uint8_t) ((data_size
        + (uint16_t) 8) & (uint8_t) 0xFF)
    };

  uint16_t address = (uint16_t) VL53LMZ_UI_CMD_END - (data_size + (uint16_t) 12) + (uint16_t) 1;
...
uint8_t vl53lmz_send_output_config_and_start(VL53LMZ_Configuration *p_dev)
  ...
  for (i = 0; i < (uint32_t) (sizeof(g_output_config) / sizeof(uint32_t)); i++)
    ...
    if ((g_output_config[i] == (uint8_t) 0)
        || ((g_output_bh_enable[i / (uint32_t) 32] & ((uint32_t) 1 << (i % (uint32_t) 32))) == (uint32_t) 0))
      {
      continue;
      }
    ...
    if (((uint8_t) bh_ptr->type >= (uint8_t) 0x1) && ((uint8_t) bh_ptr->type < (uint8_t) 0x0d))
      {
      if (((uint16_t) bh_ptr->idx >= (uint16_t) 0x54d0) && ((uint16_t) bh_ptr->idx < (uint16_t) (0x5890)))
        {
        ...
        bh_ptr->size = (uint32_t) resolution * (uint32_t) VL53LMZ_NB_TARGET_PER_ZONE;
        }
  ...
  status |= vl53lmz_dci_write_data(p_dev, (uint8_t*) &(g_output_config), VL53LMZ_DCI_OUTPUT_LIST,
      (uint16_t) sizeof(g_output_config));

  header_config[0] = p_dev->data_read_size;
  header_config[1] = i + (uint32_t) 1;

  status |= vl53lmz_dci_write_data(p_dev, (uint8_t*) &(header_config), VL53LMZ_DCI_OUTPUT_CONFIG,
      (uint16_t) sizeof(header_config));

  status |= vl53lmz_dci_write_data(p_dev, (uint8_t*) &(g_output_bh_enable), VL53LMZ_DCI_OUTPUT_ENABLES,
  ...
  status |= WrMulti(&(p_dev->platform), VL53LMZ_UI_CMD_END - (uint16_t) (4 - 1), (uint8_t*) cmd, sizeof(cmd));
  status |= _vl53lmz_poll_for_answer(p_dev, 4, 1,
  VL53LMZ_UI_CMD_STATUS, 0xff, 0x03);

  /* Read ui range data content and compare if data size is the correct one */
  status |= vl53lmz_dci_read_data(p_dev, (uint8_t*) p_dev->temp_buffer, 0x5440, 12);
...
uint8_t vl53lmz_add_output_block(VL53LMZ_Configuration *p_dev, uint32_t block_header)
  ...
  for (i = 0; i < (uint8_t) NUM_OUTPUT_CONFIG_WORDS; i++)
    ...
    || (g_output_config[i] == block_header))
      { /* OR, block already exists in list */
      break;
      }
    }
...
uint8_t vl53lmz_disable_output_block(VL53LMZ_Configuration *p_dev, uint32_t block_header)
  ...
  for (i = 0; i < NUM_OUTPUT_CONFIG_WORDS; i++)
    ...
    if ((g_output_config[i] == VL53L5_NULL_BH) || (g_output_config[i] == block_header))
      {
      break;
      }
    }
...
uint8_t vl53lmz_results_extract_block(VL53LMZ_Configuration *p_dev, uint32_t blk_index, uint8_t *p_data,
  ...
  for (i = 16; i < p_dev->data_read_size;)
    ...
    if (((uint32_t) bh_ptr->type > (uint32_t) 0x1) && ((uint32_t) bh_ptr->type < (uint32_t) 0xd))
      {
