...
void print_encoded_frame(int log_level) {
    ...
    if (log_level >= 3) {
        ...
        pbuffer += Encode_int16(pbuffer,
                (uint16_t) (SPD_Data.TRK_Data.tkd_Obj[SPD_Data.userID].speed_norm / 278 * (1 << 4))); //convert mm/s to km/h -> *(3600/1000000) = /278  // signed 12.4
...
void print_user(void) {
    ...
    uart_printf("Main User position: X=%d mm, Y=%d mm, Z=%d mm\n",
            (int16_t) SPD_Data.TRK_Data.tkd_Obj[SPD_Data.userID].CoM_pos.x,
            (int16_t) SPD_Data.TRK_Data.tkd_Obj[SPD_Data.userID].CoM_pos.y,
            (int16_t) SPD_Data.TRK_Data.tkd_Obj[SPD_Data.userID].CoM_pos.z);

    uart_printf("Nb users:%d, Intruder Alert:%d\n\n", SPD_Data.USR_Data.nb_of_users, SPD_Data.USR_Data.intruder_alert);
    for (int i = 1; i < SPD_Data.USR_Data.nb_of_users; i++) {
        uart_printf("Other User #%d, X:%d, Y:%d, Z:%d\n", i, SPD_Data.USR_Data.user_properties[i].obj_prop.CoM_pos.x,
                SPD_Data.USR_Data.user_properties[i].obj_prop.CoM_pos.y,
                SPD_Data.USR_Data.user_properties[i].obj_prop.CoM_pos.z);
    }

    if (SPD_Data.presence == 1) {

        uart_printf("\nValeurs brutes\n");
        for (int j = 0; j < 8; j++) {
            for (int i = 0; i < 8; i++) {
                //uart_printf("%4X  ", RangingData.signal_per_spad[i + j * 8] >> 16);
                //uart_printf("%4X  ", RangingData.distance_mm[i]);
                uart_printf("%4X  ", RangingData.distance_mm [i + j * 8]);
            }
            uart_printf("\n");
        }
    }
